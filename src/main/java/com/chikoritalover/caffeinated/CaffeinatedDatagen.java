...
public class CaffeinatedDatagen implements DataGeneratorEntrypoint {
    ...
    private static class CaffeinatedRecipeGenerator extends FabricRecipeProvider {
        ...
        public void generate(Consumer<RecipeJsonProvider> exporter) {
            ...
            ShapelessRecipeJsonBuilder.create(RecipeCategory.FOOD, ModItems.COFFEE_BERRIES, 9).input(ModBlocks.COFFEE_BERRY_CRATE).criterion(hasItem(ModBlocks.COFFEE_BERRY_CRATE), conditionsFromItem(ModBlocks.COFFEE_BERRY_CRATE)).offerTo(exporter);

            ShapelessRecipeJsonBuilder.create(RecipeCategory.MISC, ModItems.COFFEE_BEANS).input(ModItems.COFFEE_BERRIES).group("coffee_beans").criterion(hasItem(ModItems.COFFEE_BERRIES), conditionsFromItem(ModItems.COFFEE_BERRIES)).offerTo(exporter, new Identifier(Caffeinated.MODID, "coffee_beans_from_coffee_berries"));

            ShapedRecipeJsonBuilder.create(RecipeCategory.MISC, ModItems.GROUND_COFFEE).input('#', ModItems.COFFEE_BEANS).pattern("###").group("ground_coffee").criterion(hasItem(ModItems.COFFEE_BEANS), conditionsFromItem(ModItems.COFFEE_BEANS)).offerTo(exporter, new Identifier(Caffeinated.MODID, "ground_coffee_from_coffee_beans"));

            ShapelessRecipeJsonBuilder.create(RecipeCategory.MISC, ModItems.JAVA_BANNER_PATTERN).input(Items.PAPER).input(ModItems.COFFEE_BOTTLE).criterion(hasItem(ModItems.COFFEE_BOTTLE), conditionsFromItem(ModItems.COFFEE_BOTTLE)).offerTo(exporter);

            ShapedRecipeJsonBuilder.create(RecipeCategory.FOOD, ModItems.TIRAMISU, 2).input('#', ModItems.COFFEE_BOTTLE).input('C', Items.COCOA_BEANS).input('E', Items.EGG).input('M', Items.MILK_BUCKET).input('W', Items.WHEAT).pattern("C#C").pattern("EME").pattern("WWW").group("tiramisu").criterion(hasItem(ModItems.COFFEE_BOTTLE), conditionsFromItem(ModItems.COFFEE_BOTTLE)).offerTo(exporter);
        }
    }
}
